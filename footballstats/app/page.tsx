...
function HomeContent() {
  ...
  const [activeTab, setActiveTab] = useState<'all' | 'live' | 'upcoming' | 'today' | 'finished'>('all')
  ...
  useEffect(() => {
    ...
    const fetchMatches = async () => {
      try {
        ...
        console.log('📅 Date ranges:', { currentDateStr, weekFromNow, weekAgo })
        
        // Fetch multiple sources for comprehensive data - handle each separately
        const requests = [
          fetch(`/api/football?endpoint=/matches&dateFrom=${currentDateStr}&dateTo=${weekFromNow}`),
          fetch('/api/football?endpoint=/matches&status=SCHEDULED'),
          fetch('/api/football?endpoint=/matches'),
          fetch(`/api/football?endpoint=/matches&status=FINISHED&dateFrom=${weekAgo}&dateTo=${currentDateStr}`)
        ]
        ...
        console.log('🔍 First few matches:', combinedMatches.slice(0, 3).map(m => ({ 
          ...
          status: m.status,
          utcDate: m.utcDate,
          dateOnly: m.utcDate?.split('T')[0]
        })))
        
        // Debug: Show all unique dates in matches
        const uniqueDates = [...new Set(combinedMatches.map(m => m.utcDate?.split('T')[0]).filter(Boolean))]
        console.log('📅 All match dates found:', uniqueDates.sort())
  ...
  const getFilteredMatches = () => {
    ...
    // Filter by date range - only apply if not showing all matches
    const now = new Date()
    const today = new Date(now.getFullYear(), now.getMonth(), now.getDate())
    const weekFromNow = new Date(now.getTime() + 7 * 24 * 60 * 60 * 1000)
    const monthFromNow = new Date(now.getTime() + 30 * 24 * 60 * 60 * 1000)
    
    // Only apply date filters if activeTab is not 'all' or if specific dateRange is selected
    if (activeTab !== 'all' || filters.dateRange !== 'all') {
    ...
    if (activeTab === 'live') {
      ...
      console.log('🗓️ Today filter debug:', {
        todayStr,
        totalMatchesBeforeFilter: filtered.length,
        allMatchDates: [...new Set(filtered.map(m => m.utcDate.split('T')[0]))].sort(),
        sampleMatchDates: filtered.slice(0, 10).map(m => ({
          id: m.id,
          teams: `${m.homeTeam?.name} vs ${m.awayTeam?.name}`,
          utcDate: m.utcDate,
          dateOnly: m.utcDate.split('T')[0],
          isToday: m.utcDate.split('T')[0] === todayStr
        }))
      })
      filtered = filtered.filter(match => {
      ...
      console.log('🗓️ After today filter:', filtered.length, 'matches')
      if (filtered.length === 0) {
        console.warn('⚠️ No matches found for today. Try selecting "Tất cả" tab or "Sắp diễn ra" to see other matches.')
      }
    }
      ...
      <div className="min-h-screen bg-gradient-to-br from-slate-950 via-slate-900 to-slate-950">
        ...
        <main className="container mx-auto px-4 py-6 md:py-8">
          <div className="text-center py-12 animate-in fade-in slide-in-from-bottom-4 duration-500">
            <div className="mb-4 animate-in zoom-in duration-1000" style={{ animationDelay: '200ms' }}>
              <svg className="w-16 h-16 mx-auto text-red-400" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M12 9v2m0 4h.01m-6.938 4h13.856c1.54 0 2.502-1.667 1.732-2.5L13.732 4c-.77-.833-1.964-.833-2.732 0L4.082 16.5c-.77.833.192 2.5 1.732 2.5z" />
              </svg>
            </div>
            <h3 className="text-xl font-semibold text-white mb-2 animate-in slide-in-from-bottom-2 duration-700" style={{ animationDelay: '400ms' }}>Không thể kết nối API</h3>
            <p className="text-red-400 mb-4 animate-in slide-in-from-bottom-2 duration-700" style={{ animationDelay: '600ms' }}>{error}</p>
            <div className="space-y-2 text-sm text-slate-400 mb-6 animate-in slide-in-from-bottom-2 duration-700" style={{ animationDelay: '800ms' }}>
            ...
            <button 
              ...
              className="px-6 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-lg transition-all duration-300 hover:scale-105 hover:shadow-lg animate-in slide-in-from-bottom-2 duration-700"
    ...
    <div className="min-h-screen relative">
      ...
      <div className="relative z-10">
        ...
        <div className="flex justify-center gap-4 px-4">
          ...
          {/* Main Content */}
          <main className="flex-1 max-w-6xl py-6 md:py-8">
          {/* Team Search */}
          <div className="mb-6">
          ...
          <FilterBar 
            ...
            finishedCount={allMatches.filter(m => m.status === 'FINISHED').length}
            todayCount={(() => {
              const todayStr = getTodayString()
              ...
              console.log('📊 Today count debug:', {
                todayStr,
                totalMatches: allMatches.length,
                todayMatches: todayMatches.length,
                todayMatchesSample: todayMatches.slice(0, 3).map(m => ({
                  id: m.id,
                  utcDate: m.utcDate,
                  homeTeam: m.homeTeam?.name,
                  awayTeam: m.awayTeam?.name
          ...
          {isLoading ? (
            ...
            <>
              {/* Debug Info */}
              {selectedTeam && (
                <div className="mb-4 p-3 bg-blue-900/20 backdrop-blur-sm border border-blue-700 rounded-lg">
