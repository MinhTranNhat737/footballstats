...
function HomeContent() {
  ...
  useEffect(() => {
    console.log('🔧 Second useEffect running - starting API calls')
    ...
    const fetchMatches = async () => {
      try {
        ...
        console.log('📋 Valid data structure:', validData.map((data, i) => ({ 
          index: i, 
          isArray: Array.isArray(data), 
          length: data?.length, 
          firstItemStructure: data?.[0] ? Object.keys(data[0]) : null 
        })))
      ...
      <main className="container mx-auto px-4 py-6 md:py-8">
        ...
        <div className="mb-4 p-3 bg-slate-800 border border-slate-700 rounded-lg text-sm">
          ...
          <p className="text-slate-400 text-xs mt-1">
            Client-side: {typeof window !== 'undefined' ? 'Yes' : 'No'}, typeof window: {typeof window}
          </p>
          {allMatches.length > 0 && (
            <p className="text-slate-400 text-xs mt-1">
